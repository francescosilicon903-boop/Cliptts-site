...
### REC
**Records everything to WAV.** Press REC: ClipTTS reads the current clipboard content on its own and synthesizes it in full, accumulating the result instead of playing it back — no need to press PLAY. You can keep using the rest of the app while it generates in the background.

- **STOP** during recording: stops it and **saves** whatever was generated up to that point.
- **Pressing REC again** during recording: stops it and **discards everything** — no file is written. Use this if you changed your mind, not to save.

The file opens automatically in WavPlayer when done (or after STOP, if you interrupted it).
...
**What you can do:**
- Play the audio file
- Adjust speed (default: 1x) — presets 0.9x, 1x, 1.1x, 1.2x, 1.4x, or free on the timeline
